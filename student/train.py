...
parser.add_argument('--small_image', action='store_true',
                    help='train with small input as 640x512, otherwise train with 1280x1024')
...
parser.add_argument('--check_depth_path',
                    default='/home/1Tm2/zhz/KD-MVS/KD-MVS/kdmvs_blended/outputs/00_teacher_model_kd_iter1/checked_depth_normal_version',
                    help='check_depth')
...
train_dataset = MVSDataset(args.trainpath, args.check_depth_path, args.trainlist, "train", args.n_views,
                           robust_train=True)
test_dataset = MVSDataset(args.valpath, args.check_depth_path, args.vallist, "val", args.n_views, robust_train=False)
...
def train_sample(sample, detailed_summary=False):
    ...
    #                 sample_cuda["depth_min"], sample_cuda["depth_max"],sample_cuda["cam_extrinsic"],sample_cuda["cam_intrinsic"])
    outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"],
    ...
    loss = model_loss(depth_est, depth_upsampled_est, confidences_est, depth_gt, mask, sample_cuda["depth_min"],
                      sample_cuda["depth_max"], args.regress)
    loss.backward()

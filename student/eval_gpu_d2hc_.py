...
parser.add_argument('--img_wh', nargs='+', type=int, default=[2560, 1920],
...
parser.add_argument('--photo_thres', type=float, default=0.15, help='threshold for photometric consistency filtering')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,
    ...
    masks = []
    for i in range(2, 11):
        mask = cp.logical_and(dist < i / 4, relative_depth_diff < i / 1300)
...
def filter_depth(scan_folder, out_folder, plyfilename, geo_pixel_thres, geo_depth_thres, photo_thres, img_wh,
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            # print('start check_geometric_consistency')
            masks, geo_mask, depth_reprojected, _, _ = check_geometric_consistency(ref_depth_est, ref_intrinsics,
                                                                            ref_extrinsics,
                                                                            src_depth_est,
        ...
        for i in range(2, dy_range):
            geo_mask = cp.logical_or(geo_mask, geo_mask_sums[i - 2] >= i)

...
class Pipeline(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_min, depth_max, extrinsics, intrinsics, n_views):

        extrinsics = torch.stack(extrinsics, dim=1)[:, :, 0:-1, :]
        intrinsics = torch.stack(intrinsics, dim=1)
        img_tensor = imgs['level_0']

        # print('img_tensor')
        # print(img_tensor.shape)
        # img_tensor = (img_tensor + 1) * 255
        # img_tensor = img_tensor[0:1, 0:1, :, :].view(3, img_tensor.shape[3], img_tensor.shape[4]).permute(1, 2, 0)
        # # img_tensor = (img_tensor + 1) * 255
        # print(img_tensor.cpu().numpy().shape)
        # print(cv2.imwrite('/home/god/mvs/IterMVS-main_lstm_mutiscale_refiment_no_is_refine_new_aanet_copy/ref_yuantu.jpg',
        ...
        if not self.test:
            depths, depths_upsampled, confidences, confidence_upsampled = self.iter_mvs(ref_feature, src_features,
                                                                                        ref_proj, src_projs, depth_min,
                                                                                        depth_max, extrinsics,
                                                                                        intrinsics, img_tensor, n_views)

            return {
                "depths": depths,
                "depths_upsampled": depths_upsampled,
                # "depths_upsampled2": depths_upsampled2,
                "confidences": confidences,
                "confidence_upsampled": confidence_upsampled,
            }
        else:
            depth, depths_upsampled, depths_upsampled2, confidence, confidence_upsampled = self.iter_mvs(ref_feature, src_features,
                                                                                      ref_proj, src_projs, depth_min,
                                                                                      depth_max, extrinsics, intrinsics,
                                                                                      img_tensor, n_views)

            return {
                # "depths_upsampled": depths_upsampled,
                "depths_upsampled": depths_upsampled2,
                "confidence_upsampled": confidence_upsampled,
            }


def full_loss(depths, depths_upsampled, confidences, depths_gt, mask, depth_min, depth_max, regress=True):
    # print('depths["combine"]')
    # print(len(depths["combine"]))
    # print('depths')
    # print(len(depths))
    # print('depths_upsampled')
    # print(len(depths_upsampled))
    # exit()
    loss = 0
    radius = 4
    out_num_samples = 256
    ...
    depth = depths["initial"][0]  # depths["initial"][0] 为初始化深度图
    normalized_depth = depth_normalization(depth, inverse_depth_min, inverse_depth_max)
    loss = loss + coff * out_num_samples * F.l1_loss(normalized_depth[mask_1], normalized_depth_gt[mask_1],
                                                     reduction='mean')
    # *************************************Initial Loss************************************************* #
    ...
    for iter in range(num_prediction):
        ...
        if regress:
            ...
            if torch.sum(mask_new) > 0:
                loss = loss + coff * out_num_samples * F.l1_loss(normalized_depth[mask_new],
                                                                 normalized_depth_gt[mask_new], reduction='mean')

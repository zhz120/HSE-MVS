...
class LSTM(nn.Module):
    def __init__(self, hidden_dim, input_dim, kernel_size=3):
        ...
        self.conv_it = nn.Conv2d(hidden_dim + input_dim, hidden_dim, kernel_size, padding=kernel_size - 1, dilation=2)
        self.conv_c_t = nn.Conv2d(hidden_dim + input_dim, hidden_dim, kernel_size, padding=kernel_size - 1, dilation=2)
        self.conv_ft = nn.Conv2d(hidden_dim + input_dim, hidden_dim, kernel_size, padding=kernel_size - 1, dilation=2)
        self.conv_ot = nn.Conv2d(hidden_dim + input_dim, hidden_dim, kernel_size, padding=kernel_size - 1, dilation=2)
        # print('hidden_dim')
        # print(hidden_dim)
        # print('input_dim')
        # print(input_dim)

    def forward(self, h, c, *x_list):  # h 上一层的输入 z update r reset cr->bf cz->bi cq->bc bo
        # print('len:x_list')
        # print(len(x_list))
        ...
        # print(hx.shape)
        # TODO
        ft = torch.sigmoid(self.conv_ft(hx))  # bf是bias ft = forget_gate
        it = torch.sigmoid(self.conv_it(hx))
        c_t = torch.tanh(self.conv_c_t(hx))
        ct = c * ft + it * c_t
        ot = torch.sigmoid(self.conv_ot(hx))
        ht = ot * torch.tanh(ct)
        return ht, ct

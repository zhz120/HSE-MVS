...
def focal_loss(prob_volume, depth_gt, mask, depth_value, depth_interval, return_prob_map=False):
    ...
    cross_entropy_image = -(torch.clamp(positive_volume.log(), min=-100) * ((1 - positive_volume) ** 2)).squeeze(
        1)  # B, H, W
...
def kl_distance(pred_mu, pred_sigma, mu, sigma):
    ...
    '''
    dis = (0.5 * (torch.log(sigma / pred_sigma) + (
                pred_sigma + (pred_mu - mu) ** 2) / sigma - 1.0))  # .sum(dim=1).mean() # B,1,H,W
    return dis
...
def kl_distance_sp(pred_mu, pred_sigma, mu, sigma):
    ...
    for i, pred_mu in enumerate(pred_mu):
        kl_loss += (0.5 * (torch.log(sigma / pred_sigma[i].exp()) + (
                    pred_sigma[i].exp() + (pred_mu - mu[:, i:i + 1]) ** 2) / sigma - 1.0))  # .sum(dim=1).mean()
    return kl_loss
...
def kl_loss(prob_volume, gt_mu, gt_sigma, mask, depth_value):
    ...
    valid_pixel_num = torch.sum(mask_true, dim=[0, 1, 2]) + 1e-6  # 可能是bug，之后修改一次再试验
    ...
    pred_sigma = pred_sigma.unsqueeze(1)

    kl_loss_img = kl_distance(pred_mu + 1e-6, pred_sigma + 1e-6, gt_mu + 1e-6,
                              gt_sigma + 1e-6)  # / float(depth_num)    # B,1,H,W
    mask_true = mask_true.unsqueeze(1)
    masked_kl_loss_image = torch.mul(mask_true, kl_loss_img)  # valid pixel
    # print("masked_kl_loss_image.shape")
    # print(masked_kl_loss_image.shape)

    masked_kl_loss_image = torch.sum(masked_kl_loss_image, dim=[0, 1, 2])  # 可能是bug，之后修改一次再试验
    # print("masked_kl_loss_image.shape")
    # print(masked_kl_loss_image.shape)
    # exit()

    kl_loss_value = torch.mean(masked_kl_loss_image / valid_pixel_num)
    # kl_loss_value = torch.mean(masked_kl_loss_image.view(-1) / torch.sum(valid_pixel_num))
...
class unsup_loss(nn.Module):
    ...
    def forward(self, inputs, imgs, sample_cams, num_views=5, **kwargs):
        ...
        total_loss = torch.tensor(0.0, dtype=torch.float32, device=inputs['stage1']["depth"].device,
                                  requires_grad=False)
        total_photo_loss = torch.tensor(0.0, dtype=torch.float32, device=inputs['stage1']["depth"].device,
                                        requires_grad=False)
        total_feature_loss = torch.tensor(0.0, dtype=torch.float32, device=inputs['stage1']["depth"].device,
                                          requires_grad=False)
        ...
        smooth_loss = torch.tensor(0.0, dtype=torch.float32, device=inputs['stage1']["depth"].device,
                                   requires_grad=False)
...
def get_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, device, dtype, shape,
    ...
    if cur_depth.dim() == 2:
        ...
        if use_inverse_depth is False:
            new_interval = (cur_depth_max - cur_depth_min) / (
                        ndepth - 1)  # (B, )  Shouldn't cal this if we use inverse depth
